#!/usr/bin/env node
/** One-shot batch writer for content/problems/701-800.json */
const fs = require('fs');
...
const meta = Object.fromEntries(
  catalog.problems.filter(p => p.id >= 701 && p.id <= 800).map(p => [p.id, p])
);

const CONTENT = require('./data/problems-701-800');
