...
const skip = new Set([206, 238, 242, 283, 322, 387]);
const catalog = require('../data/catalog.json');
const problems = catalog.problems.filter(p => p.id >= 201 && p.id <= 300 && !skip.has(p.id));

const header = `/* LC #201–300 (skip 206→lc206.js, 238/242/283/322/387→lc-patterns.js) */
...
function add(id, title, code) {
    blocks.push(`    /* ${id} ${title} */\n${code}`);
}

add(201, 'Bitwise AND of Numbers Range', `    reg(201, {
        initialize(s, log, cv) {
            s.m = 5; s.n = 7;
...
add(203, 'Remove Linked List Elements', `    reg(203, {
        initialize(s, log, cv) {
...
add(205, 'Isomorphic Strings', `    reg(205, {
        initialize(s, log, cv) {
            s.s = "egg"; s.t = "add";
...
add(207, 'Course Schedule', `    reg(207, {
        initialize(s, log, cv) {
...
add(208, 'Implement Trie (Prefix Tree)', `    reg(208, {
        initialize(s, log, cv) {
...
add(209, 'Minimum Size Subarray Sum', `    reg(209, {
        initialize(s, log, cv) {
...
add(210, 'Course Schedule II', `    reg(210, {
        initialize(s, log, cv) {
            s.numCourses=4; s.edges=[[1,0],[2,0],[3,1],[3,2]];
            if (cv&&cv.str){ const p=String(cv.str).split("|"); s.numCourses=+p[0]||4; if(p[1]) s.edges=p[1].split(";").map(e=>e.split(",").map(Number)); }
...
add(211, 'Design Add and Search Words Data Structure', `    reg(211, {
        initialize(s, log, cv) {
...
add(212, 'Word Search II', `    reg(212, {
        initialize(s, log, cv) {
...
add(213, 'House Robber II', `    reg(213, {
        initialize(s, log, cv) {
...
add(214, 'Shortest Palindrome', `    reg(214, {
        initialize(s, log, cv) {
...
add(215, 'Kth Largest Element in an Array', `    reg(215, {
        initialize(s, log, cv) {
...
add(216, 'Combination Sum III', `    reg(216, {
        initialize(s, log, cv) {
...
add(217, 'Contains Duplicate', `    reg(217, {
        initialize(s, log, cv) {
...
add(218, 'The Skyline Problem', `    reg(218, {
        initialize(s, log, cv) {
...
add(219, 'Contains Duplicate II', `    reg(219, {
        initialize(s, log, cv) {
...
add(220, 'Contains Duplicate III', `    reg(220, {
        initialize(s, log, cv) {
...
add(221, 'Maximal Square', `    reg(221, {
        initialize(s, log, cv) {
...
add(222, 'Count Complete Tree Nodes', `    reg(222, {
        initialize(s, log, cv) {
...
add(223, 'Rectangle Area', `    reg(223, {
        initialize(s, log, cv) {
...
add(224, 'Basic Calculator', `    reg(224, {
        initialize(s, log, cv) {
...
add(225, 'Implement Stack using Queues', `    reg(225, {
        initialize(s, log, cv) {
...
add(226, 'Invert Binary Tree', `    reg(226, {
        initialize(s, log, cv) {
...
add(227, 'Basic Calculator II', `    reg(227, {
        initialize(s, log, cv) {
...
add(228, 'Summary Ranges', `    reg(228, {
        initialize(s, log, cv) {
...
add(229, 'Majority Element II', `    reg(229, {
        initialize(s, log, cv) {
...
add(230, 'Kth Smallest Element in a BST', `    reg(230, {
        initialize(s, log, cv) {
...
add(232, 'Implement Queue using Stacks', `    reg(232, {
...
add(233, 'Number of Digit One', `    reg(233, {
...
add(234, 'Palindrome Linked List', `    reg(234, {
...
add(235, 'Lowest Common Ancestor of a Binary Search Tree', `    reg(235, {
...
add(236, 'Lowest Common Ancestor of a Binary Tree', `    reg(236, {
...
add(237, 'Delete Node in a Linked List', `    reg(237, {
...
add(239, 'Sliding Window Maximum', `    reg(239, {
...
add(240, 'Search a 2D Matrix II', `    reg(240, {
        initialize(s, log, cv) {
...
add(241, 'Different Ways to Add Parentheses', `    reg(241, {
...
add(257, 'Binary Tree Paths', `    reg(257, {
...
add(260, 'Single Number III', `    reg(260, {
...
add(262, 'Trips and Users', `    reg(262, {
...
add(264, 'Ugly Number II', `    reg(264, {
...
add(268, 'Missing Number', `    reg(268, {
...
add(273, 'Integer to English Words', `    reg(273, {
...
add(278, 'First Bad Version', `    reg(278, {
...
add(279, 'Perfect Squares', `    reg(279, {
...
add(282, 'Expression Add Operators', `    reg(282, {
...
add(284, 'Peeking Iterator', `    reg(284, {
...
add(287, 'Find the Duplicate Number', `    reg(287, {
...
add(295, 'Find Median from Data Stream', `    reg(295, {
...
add(297, 'Serialize and Deserialize Binary Tree', `    reg(297, {
...
add(299, 'Bulls and Cows', `    reg(299, {
...
add(300, 'Longest Increasing Subsequence', `    reg(300, {
...
const outPath = path.join(__dirname, '../visualizers/lc201-300.js');
const body = blocks.join('\n\n');
fs.writeFileSync(outPath, header + body + footer);
const ids = problems.map(p => p.id).sort((a,b)=>a-b);
const regCount = (body.match(/reg\(\d+/g) || []).length;
console.log('Wrote', outPath);
console.log('Expected', problems.length, 'Registered', regCount);
