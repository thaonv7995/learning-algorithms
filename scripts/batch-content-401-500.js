#!/usr/bin/env node
/** One-shot batch writer for content/problems/401-500.json */
const fs = require('fs');
...
const meta = Object.fromEntries(
  catalog.problems.filter(p => p.id >= 401 && p.id <= 500).map(p => [p.id, p])
);

const CONTENT = require('./data/problems-401-500-content.js');
