#!/usr/bin/env node
/** One-shot batch writer for content/problems/301-400.json */
const fs = require('fs');
...
const meta = Object.fromEntries(
  catalog.problems.filter(p => p.id >= 301 && p.id <= 400).map(p => [p.id, p])
);

const CONTENT = require('./data/problems-301-400-content.js');
